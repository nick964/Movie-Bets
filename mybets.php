...
try 
    ...
    $sql = "SELECT movies.title, movies.critic_score, movies.line, movies.link, bets.bet_amt, bets.status, bets.ou from movies, bets where bets.movieid = movies.movieid and bets.userid = :userid";
    ...
    //make sure there are entries in the bets table
        ...
        <div class="row">
         <div class="col-lg-12">
             ...
             if ($activebets) {
                    ...
                    $total = 0;
                    while ($i < count($rows)) {
                        echo betstable($rows[$i]['link'], $rows[$i]['title'], $rows[$i]['line'], $rows[$i]['critic_score'], $rows[$i]['bet_amt'], $rows[$i]['status'], $rows[$i]['ou']);
                        $i++;
                    } //end loop

                } else {
                    echo "<tr><th>No bets to display!</th></tr>";
                }
